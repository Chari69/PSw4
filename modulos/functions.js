...
module.exports = (client) => {
    client.loadCommand = (commandName) => {
        try {
            client.logger.log("info", `Cargando Comando: ${commandName}.`);
            ...
            return `No se ha podido cargar el comando ${commandName}: ${e}`;
        }
    };
    ...
    client.unloadCommand = async (commandName) => {
        ...
        if (!command) return `El comando \`${commandName}\` parece que no existe, tampoco no es un alias. Intentalo de nuevo!`;
        if (command.shutdown) {
    ...
    // `client.playSound(sound, message);` para reproducir un sonido en un canal de voz
    client.playSound = async (sound, message) => {
        if (message.member.voice.channel) {
            if (!message.guild.me.permissions.has("CONNECT") || !message.member.voice.channel.permissionsFor(message.guild.me).has("CONNECT")) return message.reply("No puedo unirme a este canal de voz!");
